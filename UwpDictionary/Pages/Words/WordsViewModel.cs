...
namespace UwpDictionary.Pages.Words
{
    public class WordsViewModel : ObservableObject, IDisposable
    {
		...
		public void RefreshSelected(Action<Word> onResult)
		{
			if (_selected == null)
				return;
			SetSelected(_selected.Id,onResult);
		}

        public void SetSelected(int id, Action<Word> onResult)
        {
            Task.Run(async () =>
            {
        ...
        private void AddHistory(WordUi word, int id)
        {
            Task.Run(async () =>
            {
                ...
                if (found != null) _ = _localContext.Histories.Remove(found);
